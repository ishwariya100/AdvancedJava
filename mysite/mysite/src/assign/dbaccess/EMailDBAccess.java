...
//Getting Connection using MS Access
	...
	public static Connection getConnection() throws Exception{
		...
		dbConn = DriverManager.getConnection(url, "root", "mysql");
		}
...
// EMailAddress Queries
	...
	public EMailAddressVOO getEMailAddress(String inEMailID) throws Exception{
		...
		pStmt = dbConn.prepareStatement("SELECT * FROM emailaddress WHERE emailid = ?");
	...
	public void updateEMailAddress(EMailAddressVOO inEMailAddress) throws Exception{
	    ...
	    int rowsUpdated = 0;
	     ...
	     try{
		dbConn = getConnection();
		pStmt = dbConn.prepareStatement("UPDATE emailaddress SET fname = ?, mname = ?, lname = ?, hphone = ?, wphone = ?, mphone = ?, groupId = ? WHERE emailid = ?");
	...
	public void deleteEMailAddress(String inEMailID) throws Exception{
	    ...
	    int rowsDeleted = 0;
	     ...
	     try{
		dbConn = getConnection();
		pStmt = dbConn.prepareStatement("DELETE FROM emailaddress WHERE emailid = ?"); 
		pStmt.setString(1, inEMailID);
	...
	public ArrayList<EMailAddressVOO> getAllEMailAdressEntriesbyGroup(String inGroupID) throws Exception{
	    ...
	    System.out.println("groupId is "+ inGroupID);
	     try{
		dbConn = getConnection();
		...
		pStmt.setString(1, inGroupID);
   		...
   		if (columnCount > 0) {
   		    System.out.println("The result set has columns.");
   		}
